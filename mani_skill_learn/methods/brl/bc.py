...
class BC(BaseAgent):
    def __init__(self, policy_cfg, obs_shape, action_shape, action_space, batch_size=128):
        ...
        self.sheduler = torch.optim.lr_scheduler.StepLR(self.policy_optim, step_size=300000, gamma=0.2) #bs=128,
        #print(action_shape)
        if action_shape == 13:
        ...
        #summary(self.policy, input_size=(1200,3), batch_size=-1)
    ...
    def update_parameters(self, memory, updates):
        ...
        result = self.policy(sampled_batch['obs'], mode='eval')
        if type(result) is tuple:
            pred_action, pred_next_ee_pose = result
        else:
            pred_action = result
...
@BRL.register_module()
class BC_old(BaseAgent):
    def __init__(self, policy_cfg, obs_shape, action_shape, action_space, batch_size=128):
        super(BC_old, self).__init__()
        self.batch_size = batch_size
        self.use_osc = False
        policy_optim_cfg = policy_cfg.pop("optim_cfg")
...
@BRL.register_module()
class BC_gaussion(BaseAgent):
    def __init__(self, policy_cfg, obs_shape, action_shape, action_space, batch_size=128):
        super(BC_gaussion, self).__init__()
        self.batch_size = batch_size
        self.use_osc = False
        policy_optim_cfg = policy_cfg.pop("optim_cfg")
        ...
        self.policy = build_model(policy_cfg)
        self.policy_optim = build_optimizer(self.policy, policy_optim_cfg)
        self.sheduler = torch.optim.lr_scheduler.StepLR(self.policy_optim, step_size=200000, gamma=0.2) #bs=128,
...
@BRL.register_module()
class BC_2(BaseAgent):
    def __init__(self, policy_cfg, obs_shape, action_shape, action_space, batch_size=128):
        super(BC_2, self).__init__()
        self.batch_size = batch_size
        self.use_osc = False
        policy_optim_cfg = policy_cfg.pop("optim_cfg")
    ...
    def update_parameters(self, memory, updates):
        sampled_batch = memory.sample(self.batch_size)
        sampled_batch = dict(obs=sampled_batch['obs'], actions=sampled_batch["actions"], prev_obs=sampled_batch['prev_obs'])
        #print("bc",sampled_batch['next_obs']['state'].shape)
        sampled_batch = to_torch(sampled_batch, device=self.device, dtype='float32')
        for key in sampled_batch:
            if not isinstance(sampled_batch[key], dict) and sampled_batch[key].ndim == 1:
        ...
        sampled_batch['obs']['pointcloud']['xyz'] = torch.cat((sampled_batch['obs']['pointcloud']['xyz'], sampled_batch['prev_obs']['pointcloud']['xyz']), dim=-2) #B 2400 3
        sampled_batch['obs']['pointcloud']['rgb'] = torch.cat(
            (sampled_batch['obs']['pointcloud']['rgb'], sampled_batch['prev_obs']['pointcloud']['rgb']),
            dim=-2)  # B 2400 3
        sampled_batch['obs']['pointcloud']['seg'] = torch.cat(
            (sampled_batch['obs']['pointcloud']['seg'], sampled_batch['prev_obs']['pointcloud']['seg']),
            dim=-2)  # B 2400 3
        pred_action = self.policy(sampled_batch['obs'], mode='eval')
