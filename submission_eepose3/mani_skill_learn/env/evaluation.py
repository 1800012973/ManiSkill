...
class Evaluation:
    ...
    def step(self, action):
        ...
        if self.save_traj:
            ...
            """
            from mymani_skill_learn.utils.data import get_shape_and_type
    ...
    def run(self, pi, num=1, work_dir=None, **kwargs):
        ...
        while self.episode_id < num:
            ...
            with torch.no_grad():
                action = to_np(pi(unsqueeze(obs, axis=0), mode=self.sample_mode)[0])[0]
    ...
    def run(self, pi, num=1, work_dir=None, **kwargs):
        ...
        if self.synchronize:
            ...
            while True:
                ...
                obs = self.recent_obs
                with torch.no_grad():
                    actions = to_np(pi(obs, mode='eval')[0])
                for i in range(n):
