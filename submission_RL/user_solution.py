...
from mani_skill_learn.methods.builder import build_brl, build_mfrl
...
class UserPolicy(BasePolicy):
    def __init__(self, env_name):
        ...
        self.obs_mode = 'state'  # remember to set this!
        self.env.set_env_mode(obs_mode=self.obs_mode)
        self.stack_frame = 1


        cfg_path = str(pathlib.Path('./configs/sac/sac_mani_skill_state_1M_train.py').resolve())
        #cfg_path = './configs/bc/mani_skill_point_cloud_transformer.py'
        ...
        self.agent = build_mfrl(cfg.agent)
        load_checkpoint(self.agent,
            str(pathlib.Path('./drawer_5000000.ckpt').resolve()),
            map_location='cpu'
        )
